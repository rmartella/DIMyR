...
void Model::render(Shader * shader) {
	...
	for (GLuint i = 0; i < this->meshes.size(); i++) {
		...
		this->vecBones[i].bonesTransform(runningTime, transforms, scene);
		for (uint i = 0; i < transforms.size(); i++) {
			std::stringstream ss;
			ss << "bones[" << i << "]";
			int matrixLocation = shader->getUniformLocation(ss.str());
			glUniformMatrix4fv(matrixLocation, 1, GL_TRUE,
					glm::value_ptr(
							glm::transpose(
									m_GlobalInverseTransform * transforms[i])));
		}
...
void Model::loadModel(const std::string & path) {
	...
	this->scene = importer.ReadFile(path.c_str(),
...
// Process each mesh located at the current node
	for (GLuint i = 0; i < node->mNumMeshes; i++) {
		...
		Bones bones(this->meshes[i].getVao(), mesh->mNumVertices);
		bones.loadBones(i, mesh);
		vecBones.push_back(bones);
	}
