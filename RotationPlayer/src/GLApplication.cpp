...
void GLApplication::applicationLoop() {
	...
	while (processInput) {
		...
		if (windowManager->inputManager.getKeyState()[InputCodes::Up]) {
			currPosition.z = currPosition.z + 0.01 * glm::cos(currAngle);
			currPosition.x = currPosition.x + 0.01 * glm::sin(currAngle);
		}
		if (windowManager->inputManager.getKeyState()[InputCodes::Down]) {
			currPosition.z = currPosition.z - 0.01 * glm::cos(currAngle);
			currPosition.x = currPosition.x - 0.01 * glm::sin(currAngle);
		}

		if (windowManager->inputManager.getKeyState()[InputCodes::Left]) {
			currAngle += 0.001;

			if (currAngle > 2 * M_PI)
				currAngle -= 2 * M_PI;
			else if (currAngle < -2 * M_PI)
				currAngle += 2 * M_PI;
		}

		if (windowManager->inputManager.getKeyState()[InputCodes::Right]) {
			currAngle -= 0.001;
		...
		localModelMatrix = glm::translate(model, currPosition);
		localModelMatrix = glm::rotate(localModelMatrix, currAngle, glm::vec3(0.0, 1.0, 0.0));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(localModelMatrix));
